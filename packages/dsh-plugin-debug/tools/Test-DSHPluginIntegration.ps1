...
function Assert-PluginIntegration {
...
try {
  ...
  Assert-PluginIntegration ($package.name -ceq 'dsh-plugin-debug') 'combined package name is not dsh-plugin-debug'
  Assert-PluginIntegration ($package.files -contains 'Start-DSH-Combined.ps1' -and $package.files -contains 'Start-DSH-Combined.cmd' -and $package.files -contains 'Start-DSH-Combined.vbs') 'combined launcher files are missing from package.files'
  Assert-PluginIntegration (-not (Test-Path -LiteralPath (Join-Path $packageRoot 'dsh-plugin-store') -PathType Any)) 'removed plugin-store package content still exists'
  ...
  Assert-PluginIntegration ($manifest.package -ceq 'dsh-plugin-debug' -and $manifest.independentRuntime -eq $true) 'bundle manifest does not describe the combined package'
  Assert-PluginIntegration ($pointer.global -ceq '__DSH_PLUGIN_DEBUG__' -and $pointer.legacyGlobal -ceq '__DSH_PLUGIN_PROVENANCE__' -and $pointer.bridgeSelector -ceq 'meta[data-dsh-debug-bridge="1"]' -and $pointer.pointerEvent -ceq 'dsh-plugin-debug:pointer') 'bundle manifest does not expose canonical Debug and legacy provenance contracts'
  ...
  foreach ($marker in @('__DSH_PLUGIN_DEBUG__', '__DSH_PLUGIN_PROVENANCE__', 'data-dsh-debug-bridge', 'data-dsh-provenance-bridge', 'const POINTER_EVENT', 'PLUGIN_ID', 'getPointerEvidence')) {
    Assert-PluginIntegration ($clientText.Contains($marker)) "client artifact is missing bridge marker: $marker"
  }
...
'@
  ...
  Assert-PluginIntegration ($install.exitCode -eq 0) "offline single-package install failed: $($install.text)"
  ...
  Assert-PluginIntegration (Test-Path -LiteralPath $profileManifestPath -PathType Leaf) 'offline install did not create the Profile manifest'
  $installedManifest = Get-Content -LiteralPath $profileManifestPath -Raw -Encoding UTF8 | ConvertFrom-Json
  Assert-PluginIntegration (@($installedManifest.dsh.profile.bundles) -contains 'dsh-plugin-debug') 'Profile bundle list does not contain dsh-plugin-debug'
  Assert-PluginIntegration (Test-Path -LiteralPath (Join-Path $installedRoot 'lib\client.js') -PathType Leaf) 'offline install did not copy the combined client artifact'
  Assert-PluginIntegration (-not (Test-Path -LiteralPath (Join-Path $installedRoot 'dsh-plugin-store') -PathType Any)) 'removed plugin-store content entered the installed package'

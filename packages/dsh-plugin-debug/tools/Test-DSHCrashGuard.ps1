...
'@
  ...
  $result = [ordered]@{
    result = if ($launcherExitCode -eq 0 -and $bootCount -ge 2 -and $quarantine.Count -eq 1 -and $patchText -match "id: 'test-dsh-plugin'" -and $patchText -match 'disabled: true' -and $launcherLog -match 'DSH Web ready') { 'PASS' } else { 'FAIL' }

...
test('client error capture redacts URLs and sensitive-looking text', async () => {
  ...
  assert.equal(errors[0].filename, 'https://example.test/app.js')
  assert.doesNotMatch(errors[0].message, /secret-value/u)
  assert.doesNotMatch(errors[0].message, /bearer-secret|json-secret/u)
  assert.doesNotMatch(errors[0].stack, /token=abc/u)

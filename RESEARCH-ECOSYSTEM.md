...
| [SenmuuuuW/dsh-whale-report](https://github.com/SenmuuuuW/dsh-whale-report) | MIT；`main` 当前审计提交 `b3de4a7d8851f63757078427ecfda52bc908961f`；版本 `0.4.0`；19 stars（2026-08-17） | Session/Token/Tool Call/失败/重试/危险命令/本地费用估算/确定性洞察/报告 UI；另含余额探针、凭据读取和 DeepSeek 网络请求；构建脚本仍使用 `rm -rf lib`，CI 只跑 Ubuntu | 吸收确定性报告引擎和“覆盖范围/估算/风险线索”产品形状；当前 `dsh_agent_report` 不复制完整 UI、余额探针、凭据读取、网络价格/余额请求、上游依赖或 Unix-only 构建脚本；`rm -rf` 只作为文本检测规则，不执行 |

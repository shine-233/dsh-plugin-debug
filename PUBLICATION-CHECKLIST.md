...
- [x] In the current isolated Profile, `session.create` with the minimal preset passed. A 2026-08-18 read-only check of the user's existing `web` Profile also reproduced `agent-preset-invalid`/duplicate `deployment:persona` while resuming an existing blank Session through `session.models`; classify this as a Profile/runtime loading issue, not a Debug failure, and do not generalize it to every Profile.
...
- [x] On 2026-08-18, the current documentation-only `main` was rechecked with `npm run check`: generated artifacts, 35 JavaScript files and 13 pinned external workflow uses across 3 workflows were verified; all 95 Node tests passed and the worktree remained clean.
- [x] Post-release CI run `32147499810` and CodeQL run `32147499705` both completed with `success` for documentation-only commit `9b5cf189d295c83ad8bc55fe28222da04e2ea15e`; the CI run included Node 22/24, the complete Windows debug suite, a fresh clone, real prepack tarball extraction and package smoke checks. The immutable `v0.8.4` tag and GitHub Release were not moved.
- [x] The user's existing `web` Profile was started on a fresh loopback port with `-NoInstall -NoPluginInstall` and no model request. HTTP 200, `host.describe`, 134 inventory entries and active `dsh-plugin-debug` all passed; Profile manifest/patch SHA-256 values were unchanged and the exact started process was stopped through its PID record.
- [x] Third-party candidates were rechecked without installing dependencies: `dsh-plugin-check` passed typecheck and 81/81 tests; `dsh-whale-report` shipped parseable JavaScript but cannot load in the research clone because declared `zod` is absent; Hongzhong's hotswap entry parses, while Jarvan's declared `lib/index.js` is absent.
- [x] Both hotswap candidates still fail closed to `MANUAL_REVIEW` under Debug's strict offline preflight. Observed blockers include shell/package-manager execution, private lifecycle APIs, patch/watcher mutation and missing CI; the candidates additionally lack combinations of tests, atomic writes, authentication, rollback and core protection. No candidate code or hotswap mutation was executed.
